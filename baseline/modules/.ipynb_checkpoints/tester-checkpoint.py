        ...
        with open(os.path.join(self.save_dir, self.split + '.json'), "w") as outfile:
            with torch.no_grad():
                ...
                for topic_t in self.args.topic_type:
                    ...
                    topic_gts = {i: [v] for i, v in enumerate([v for i, v in enumerate(test_gts) if i in index_])}
                    topic_res = {i: [v] for i, v in enumerate([v for i, v in enumerate(test_res) if i in index_])}
                    topic_metrics = self.metric_ftns(topic_gts, topic_res)
                    write_json["scores"].append({'test_' + topic_t + '_' + k: v for k, v in topic_metrics.items()})
                    log.update(**{'test_' + topic_t + '_' + k: v for k, v in topic_metrics.items()})

                final_gts = {i: [gt] for i, gt in enumerate(test_gts)}
                final_res = {i: [re] for i, re in enumerate(test_res)}
                test_met = self.metric_ftns(final_gts, final_res)

                # Merge into log and JSON
                log.update({'test_' + k: v for k, v in test_met.items()})
                write_json["scores"].append({'test_' + k: v for k, v in test_met.items()})

                # Show medspacy metrics separately
                print("\nMedSpaCy Evaluation:")
                print(f" - Precision: {test_met.get('medspacy_precision', 0):.4f}")
                print(f" - Recall:    {test_met.get('medspacy_recall', 0):.4f}")
                print(f" - F1 Score:  {test_met.get('medspacy_f1', 0):.4f}")
