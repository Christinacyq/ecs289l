...
class XGRenModel(nn.Module):
    def __init__(self, args, tokenizer):
        ...
        if hasattr(args, "use_rag") and args.use_rag:
            print('################################# using RAG')
            self.retriever = Retriever(args, tokenizer)
        else:
            self.retriever = None
        ...
        if mode == 'train':
            ...
            if self.retriever is not None:
                ...
                for i in range(images.size(0)):
                    retrieved = self.retriever.retrieve_report(images[i, 0])  # only from primary image
                    max_len = max(max_len, len(retrieved))
                    retrieved_token_ids.append(retrieved)
    
                for i in range(len(retrieved_token_ids)):
                    pad_len = max_len - len(retrieved_token_ids[i])
                    if pad_len > 0:
                        pad_tensor = torch.full((pad_len,), self.args.pad_idx, dtype=torch.long, device=retrieved_token_ids[i].device)
                        retrieved_token_ids[i] = torch.cat([retrieved_token_ids[i], pad_tensor], dim=0)
    
                retrieved_token_ids = torch.stack(retrieved_token_ids)
    
                # Truncate target if needed
                if targets.size(1) + retrieved_token_ids.size(1) > self.args.max_seq_length:
                    max_target_len = self.args.max_seq_length - retrieved_token_ids.size(1)
                    targets = targets[:, :max_target_len]
    
                # Prepend retrieved tokens
                targets = torch.cat([retrieved_token_ids, targets], dim=1)
            ...
            raise ValueError("Invalid mode: should be 'train' or 'sample'")
