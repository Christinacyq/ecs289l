...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, ref, register_hook=False):
        B, N, C = x.shape
        _, N_ref, _ = ref.shape

        q = self.q(ref).reshape(B, N_ref, 1, self.num_heads, C //
                                  self.num_heads).permute(2, 0, 3, 1, 4)
...
class CossBlock(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_grad_checkpointing=False):
        super().__init__()
        self.norm1_0 = norm_layer(dim)
    ...
    def forward(self, x, ref, register_hook=False):
        ref = ref + self.drop_path(self.attn(self.norm1_0(x), self.norm1_1(ref), register_hook=register_hook))
